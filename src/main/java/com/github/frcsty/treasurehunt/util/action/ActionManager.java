...
import com.github.frcsty.treasurehunt.util.action.actions.command.ConsoleCommand;
import com.github.frcsty.treasurehunt.util.action.actions.command.PermissionCommand;
...
public final class ActionManager {
    ...
    private static final Pattern RANDOM_PATTERN = Pattern.compile("\\{random:(?<values>((\\d+),?)+)}");
     ...
     */
    private ActionManager(final TreasureHuntPlugin plugin, final boolean defaults) {
        this.plugin = plugin;
    ...
    public ActionManager(final TreasureHuntPlugin plugin) {
        this(plugin, true);
    }
    ...
    private String replacePlaceholders(final Player player, final String line) {
        if (Bukkit.getPluginManager().isPluginEnabled("PlaceholderAPI")) {
            return PlaceholderAPI.setPlaceholders(player, Color.parse(line));
        }

        return StringUtils.replace(Color.parse(line), "%player_name%", player.getName());
    }
    ...
    private String replaceTags(String line) {
        ...
        while (randomMatcher.find()) {
            ...
            if (numbers.size() == 1) {
                ...
                number = RANDOM.nextInt(numbers.get(0), numbers.get(1) + 1);
            } else if (numbers.size() > 2) {
                number = numbers.get(RANDOM.nextInt(numbers.size()));
            }
